...
namespace BitBenderGames
{
  ...
  {
    private const float kNearestPositionOffset = .1f;

    public event Action OnStopScrolling;
    ...
    [SerializeField]
    [Tooltip("The lower the value, the slower the camera will follow. The higher the value, the more direct the camera will follow movement updates. Necessary for keeping the camera smooth when the framerate is not in sync with the touch input update rate.")]
    private float moveCamFollowFactor = 15.0f;
    [SerializeField]
...
#pragma warning restore 414
    ...
    private bool isMoveStateNow;
    private float timeRealDragStop;
    private float timeMoveCam = float.NaN;
    private float timeMoveCamTotal = float.NaN;

    public bool IsAutoScrolling { get { return (cameraScrollVelocity.sqrMagnitude > float.Epsilon); } }
    public Func<bool> NeedUpdate { get; set; }
    public bool IsPinching { get; private set; }
    public bool IsDragging { get; private set; }
    public bool IsEnableInput { get; set; }

    #region internal tweakables
    [Header("Expert Mode")]
    [SerializeField]
    ...
    public bool IsMoveTo { get { return isMoveState; }}
    ...
    {
      get { return isMoveStateNow; }
      set
      {
        isMoveStateNow = value;
        if (!value)
        {
          timeMoveCam = float.NaN;
        }
      }
    }
    ...
    public Vector3 GetIntersectionPoint(Ray ray)
    {
      ...
      if (success == false)
      {
        //if(Application.isEditor) Debug.LogError("Failed to compute intersection between camera ray and reference plane. Make sure the camera Axes are set up correctly.");
      }
    ...
    private void UpdatePosition(float deltaTime)
    {
      ...
      if ((IsDragging == true || IsPinching == true) && IsEnableInput)
      {
        isMoveState = false;
      ...
      if (isMoveState)
      {
        if (!float.IsNaN(timeMoveCam))
        {
          timeMoveCam -= Time.deltaTime;
          timeMoveCam = Mathf.Max(0, timeMoveCam);

          var ratio = (timeMoveCamTotal - timeMoveCam) / timeMoveCamTotal;
          var newPos = Vector3.Lerp(moveStartPos, GetClampToBoundaries(targetPositionClamped), Mathf.Clamp01(ratio));
          newPos.y = Transform.position.y;
          Transform.position = newPos;

          if (IsNearestPosition(Transform.position, targetPositionClamped))
          {
            isMoveState = false;
          }
        }
        ...
        {
          Transform.position = Vector3.Lerp(Transform.position, GetClampToBoundaries(targetPositionClamped), Mathf.Clamp01(Time.deltaTime * moveCamFollowFactor));
          if (IsNearestPosition(Transform.position, targetPositionClamped))
          {
            Transform.position = targetPositionClamped;
            isMoveState = false;
      ...
      if (IsDragging == false && IsPinching == false && !isMoveState)
      {
    ...
    private bool IsNearestPosition(Vector3 position, Vector3 targetPositionClamped)
    {
      return Mathf.Abs(position.x - targetPositionClamped.x) <= kNearestPositionOffset &&
             Mathf.Abs(position.z - targetPositionClamped.z) <= kNearestPositionOffset;
    }
    ...
    public void Update()
    {
      ...
      if (Mathf.Approximately(mouseScrollDelta, 0) == false && !EventSystemWrapper.IsPointerOverGameObject() && IsEnableInput)
      {
        if (EventSystemWrapper.IsPointerOverGameObject()) mouseScrollDelta = 0f;
      ...
      if (IsPinching == false && IsDragging == false && IsEnableInput)
      {
    ...
    {
      if (isDraggingSceneObject == false && IsEnableInput)
      {
...
#pragma warning restore 162
    ...
    private Vector3 GetClampToOverdrag(Vector3 camPos)
    {
      return camPos;
      //var margin = camOverdragMargin;
      //Vector3 result = Vector3.zero;
      //if (camPos.x < CamPosMin.x + margin)
      //{
      //  result.x = CamPosMin.x + margin;
      //}
      //else if (camPos.x > CamPosMax.x - margin)
      //{
      //  result.x = CamPosMax.x - margin;
      //}
      ...
      //    if (camPos.y < CamPosMin.y + margin)
      //    {
      //      result.y = CamPosMin.y + margin;
      //    }
      //    else if (camPos.y > CamPosMax.y - margin)
      //    {
      //      result.y = CamPosMax.y - margin;
      //    }
      //    break;
      //  case CameraPlaneAxes.XZ_TOP_DOWN:
      //    if (camPos.z < CamPosMin.y + margin)
      //    {
      //      result.z = CamPosMin.y + margin;
      //    }
      //    else if (camPos.z > CamPosMax.y - margin)
      //    {
      //      result.z = CamPosMax.y - margin;
      //    }
      //    break;
      //}
    ...
    public void StopScrolling()
    {
      cameraScrollVelocity = Vector3.zero;
      isMoveState = false;
      if (OnStopScrolling != null)
      {
        OnStopScrolling();
      }
    }

    public void MoveToPosition(Vector2 newPosition, bool immediately = false)
    {
      StopScrolling();
      var targetPosition = new Vector3(newPosition.x, Transform.position.y, newPosition.y);
      var newPositionClamped = GetClampToBoundaries(targetPosition);
      newPositionClamped = GetClampToOverdrag(newPositionClamped);
      targetPositionClamped = newPositionClamped;
      isMoveState = !immediately;
      if (immediately)
      {
        Transform.position = newPositionClamped;
      }
    }

    public void MoveToPosition(Vector2 newPosition, float time)
    {
      StopScrolling();
      var targetPosition = new Vector3(newPosition.x, Transform.position.y, newPosition.y);
      var newPositionClamped = GetClampToBoundaries(targetPosition);
      ...
      isMoveState = true;
      timeMoveCam = time;
      timeMoveCamTotal = timeMoveCam;
      moveStartPos = Transform.position;
    }

    public void MoveTo(Vector3 newPosition, bool immediately = false)
    {
      StopScrolling();
      var newPositionClamped = GetClampToBoundaries(newPosition);
      newPositionClamped = GetClampToOverdrag(newPositionClamped);
      targetPositionClamped = newPositionClamped;
      isMoveState = !immediately;
      if (immediately)
    ...
    public void MoveTo(Vector3 newPosition, float time)
    {
      StopScrolling();
      var newPositionClamped = GetClampToBoundaries(newPosition);
      newPositionClamped = GetClampToOverdrag(newPositionClamped);
