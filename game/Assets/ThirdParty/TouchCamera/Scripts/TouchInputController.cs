...
namespace BitBenderGames
{
  ...
  public class TouchInputController : MonoBehaviour
  {
    ...
    private bool lastFingerPressedOnUI;
    private bool lastFingerDownOnUILastFrame;
    ...
    public UnityEvent OnFingerDownEvent;
    public event System.Action OnFingerUp;
    public UnityEvent OnFingerUpEvent;
    ...
    public UnityEvent OnDragUpdateEvent;
    ...
    public UnityEvent OnDragStopEvent;
    ...
    private bool IsOverUI
    {
      get
      {
        ...
        return EventSystemWrapper.IsPointerOverGameObject();
      }
    }
    ...
    public void Awake()
    {
      ...
      lastFingerPressedOnUI = false;
      lastFingerDownOnUILastFrame = false;
    ...
    {
      if (!lastFingerDownOnUILastFrame && TouchWrapper.IsFingerDown)
      {
        lastFingerDownOnUILastFrame = true;
        lastFingerPressedOnUI = IsOverUI || !EventSystemWrapper.IsCurrentEnabled;
      }
          ...
          if (wasPinchingLastFrame == false)
          {
            if (wasFingerDownLastFrame == true && TouchWrapper.IsFingerDown)
            {
              if (isDragging == false)
              {
                ...
                if (OnLongTapProgress != null && !lastFingerPressedOnUI)
                {
        ...
        if (isPinching == false && isDragging == false && wasPinchingLastFrame == false && wasDraggingLastFrame == false && isClickPrevented == false)
        {
          ...
          if (wasFingerDownLastFrame == true && TouchWrapper.IsFingerDown == false)
          {
            ...
            if (wasDraggingLastFrame == false && wasPinchingLastFrame == false)
            {
              ...
              bool isDoubleClick = clickDuration < doubleclickDurationThreshold && EventSystemWrapper.CanDoubleClick();
              bool isLongTap = fingerDownUpDuration > clickDurationThreshold;

              if (OnInputClick != null && !lastFingerPressedOnUI)
              {
    ...
    {
      if (!EventSystemWrapper.CanPinch()) return;
    ...
    {
      if (OnDragStart != null && !lastFingerPressedOnUI)
      {
    ...
    {
      if (OnDragUpdate != null && !lastFingerPressedOnUI && EventSystemWrapper.CanDrag())
      {
    ...
    private void DragStop(Vector3 pos)
    {
      ...
      if (OnDragStop != null && !lastFingerPressedOnUI)
      {
    ...
    {
      isFingerDown = true;
      if (OnFingerDown != null && !lastFingerPressedOnUI)
      {
    ...
    {
      isFingerDown = false;
      if (OnFingerUp != null && !lastFingerPressedOnUI)
      {
